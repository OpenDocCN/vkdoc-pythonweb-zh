...
当该应用启动时，索引路由将根据请求返回“您好，来自 Flask！”如图 [1-1](#Fig1) 所示。
...
使用前面的命令，您应该能够运行简单的 Flask 应用，并且应该在您的终端中看到类似的输出(图 [1-10](#Fig10) )。
...
该命令执行后，shell 中的(venv)前缀将消失，如果您尝试再次运行该应用，它将抛出一个错误(图 [1-11](#Fig11) )。
