...
以我们的图书电商系统 BookTime 为例，这个库工作得很好，因为所有页面都是 Django 返回的模板。如果在您的项目中，您有一个带有后端 API 的 SPA 架构，那么这种方法就不能很好地工作，因为 Django Debug Toolbar 依赖于后端将 HTML 返回给浏览器。这样的话，你可能想看看Django丝绸。
