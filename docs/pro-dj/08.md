...
对于这个例子，我们将使用一个优秀的开源病毒扫描应用，ClamAV， [<sup>8</sup>](#Fn8) ，它是为在服务器中使用而设计的，以及 pyclamd， [<sup>9</sup>](#Fn9) 一个用于与 ClamAV 交互的 Python 库。总之，它们提供了一个易于使用的接口，可以在任何传入的文件被传递给应用的其他部分之前对其进行扫描。如果发现了病毒，可以在病毒对任何人造成伤害之前，立即将恶意文件从输入流中删除。
