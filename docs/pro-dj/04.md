...
最重要的是，URL 是应用程序用户界面的一部分，因为用户必须看到它们，阅读它们，并且经常手动输入它们。在设计你的 URL 时，请记住这一点。

### 标准 URL 配置

Django 没有提供任何为任何站点自动发现或生成 URL 结构的特性。相反，每个站点和应用程序都应该使用 URL 配置明确声明最合适的寻址方案。这不是一个限制——这是一个允许你以自己喜欢的方式定义站点地址的特性。毕竟，网站就像房地产一样；你的网络框架不应该决定你的平面图。
...
从历史上看，所有视图都被指定为字符串，因此前缀是减少从单个应用程序将 URL 映射到视图所需的重复量的好方法。最近，URL 模式被允许将视图指定为可调用的，在这种情况下，前缀将被忽略。使用前缀将视图指定为字符串仍然很有用，因为它不需要视图的一组导入，从而减少了整体代码。
...
以这种方式提供同名的多个参数是不好的，因为它严重依赖 Django 对情况的处理才能正常工作。虽然这种行为不太可能因一时兴起而改变，但依赖它可能会在未来引发问题。更重要的是，在多个地方指定相同的参数名称会大大降低 URL 配置的可读性。即使在闭源应用程序中，在你完成代码后很久，其他人也可能需要阅读你的代码。
...
正如上一节提到的，Django 提供了另一个 URL 解析过程，如果应用得当，这个过程会更有用。应用程序通常需要提供链接或重定向到应用程序的其他部分或网站上的其他地方，但直接硬编码这些链接通常不是一个好主意。毕竟，即使是专有应用程序也可以改变它们的 URL 结构，而分布式应用程序可能根本不知道 URL 结构是什么样子。
...
虽然第 1 章将模板列为一个单独的层，但是请记住，它们实际上只是 Django 提供给应用程序其他部分的一个工具，包括视图。最终，无论是否使用模板来生成内容，视图单独负责生成最终的响应。Django 的模板系统没有请求或响应的概念；它只是生成文本。剩下的就由视图来处理了。
...
请求和响应之间发生的事情在很大程度上是不受限制的，并且视图可以用于需要满足的许多目的。可以构建视图来服务于特定的目的，或者可以使它们足够通用以用于分布式应用程序。

### 将视图编写为通用视图

Django 开发中的一个常见主题是使代码尽可能地可重用和可配置，以便应用程序和代码片段在多种情况下都有用，而不必为每一种需求重写代码。这就是 DRY 的全部意义:不要重复自己。

视图对于 DRY 来说是一个挑战，因为它们只被传入的请求调用。看起来似乎不可能编写一个视图，除了它最初打算用于的请求之外，它还可以被调用。然而，Django 本身有很多通用视图的例子，这些视图可以用于各种各样的应用程序和情况，每次新的使用只需要少量的配置。

有一些准则可以极大地帮助视图的重用，使它们足够通用，可以在各种应用程序中使用。视图甚至可以如此通用，以至于可以分发给其他人，并包含在原作者没有概念的项目中。
...
这个视图将非常适合它的预期目的，但是它与一个特定的博客应用程序紧密相连。它仍然是松散耦合的，因为它不需要处理如何检索博客文章或呈现模板的细节，但仍然依赖于特定于博客应用程序的细节，如模型和模板。
...
例如，个人页面的视图必须始终验证用户是否登录以及他们是否拥有适当的权限。一个应用程序可能有十几种不同类型的视图，但是如果它们都是私有的，那么它们每次都必须使用相同的代码。幸运的是，我们正在使用 Python，这提供了一个有用的选择。
...
第 2 章展示了如何使用 decorators 编写原始函数的包装器，该包装器可以访问该函数的所有参数，以及函数本身的返回值。就视图而言，这意味着 decorators 总是可以访问传入的请求对象和传出的响应对象。在某些情况下，装饰器可以是特定应用程序的特例，这将允许它预期特定于该应用程序的大量参数。

decorators 可以提供很多视图，其中一些很常见，足以保证包含在 Django 本身中。位于`django.views.decorators`的是一些包含装饰器的包，你可以在任何应用程序的任何视图上使用它们。下面列出的包只提供了完整导入路径的尾部，因为它们都位于同一位置。
...
位于`django.contrib`的捆绑应用程序提供了额外的装饰器。这些装饰器都位于该路径之下，所以和前面的列表一样，只提供了相关的路径:
...
这些只是 Django 本身捆绑的装饰器。decorators 还有许多其他用途，第三方应用程序也可以提供它们自己的用途。然而，为了让这些装饰器有用，它们必须应用于视图。

#### 应用视图装饰器

第 2 章描述了装饰器如何应用于标准 Python 函数。将 decorators 应用于视图的工作方式是相同的，但是有一个显著的区别:视图并不总是在您的控制之下。

[第二章](02.html)描述的技术假设你修饰的功能是你自己的。虽然这是经常发生的情况，但分布式应用程序的数量意味着许多 Django 支持的网站将使用其他来源的代码，并有自己的视图。如前所述应用 decorators 需要修改第三方代码。
...
内置的管理接口使用视图函数本身的名称和 docstring 为应用程序的视图生成文档。通过使用 decorators 包装函数，我们实际上是用包装器替换了原始的视图函数。这导致管理界面看到包装器，而不是视图。
...
提供这个模型的应用程序也可以为项目提供一个装饰器，应用到它们自己的视图中，自动记录这个模型的异常。
...
类为多个方法提供了交互的机会，所以 Django 使用一些标准方法来处理常见的项目，同时为您提供了一种添加其他方法的方式，现在只需要担心您的应用程序的细节。而且因为它只是一个类，你也可以添加你认为合适的其他方法。Django 不会对它们做任何事情，所以您必须自己调用它们，但是它确实给了您一个机会，让您比使用原始函数更容易地抽象出公共代码。
...
通过允许自定义对象和装饰器用于 URL 模式和视图，几乎任何有效的 Python 代码都可以自定义 URL 如何映射到视图以及视图本身如何执行。以下只是一个尝试的可能性；其余的取决于您的应用程序的需要。
...
视图是任何应用程序的真正主力，接收用户输入并将其转化为有用的输出。虽然视图可以使用整个 Python，但是 Django 确实提供了一个非常重要的工具来处理 Web 上最常见的用户输入任务之一:表单。
