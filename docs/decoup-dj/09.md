...
这个应用有一个表单，用户可以填写字段来创建一个新的发票，然后点击创建发票在后端创建新的实体。图 [9-1](#Fig1) 显示了第 [6](06.html) 章的最终形式。
...
在这个例子中，我们指示 Cypress 将页面上类型为`number`的两个输入作为目标。有了这些知识，并着眼于我们的应用的 HTML 表单结构，我们可以将清单 [9-4](#PC14) 中所示的代码添加到我们之前的测试中。
...
很容易迷失在测试实现细节和内部代码中，但实际上，它不应该那么复杂。当决定测试什么时，您需要关注一件事:系统的预期输出。在我们的 Django 应用中，我们公开了 REST 端点。这意味着我们需要看看这个系统是如何使用的，并相应地测试这些界限。在识别了系统的表面之后，对内部逻辑的测试自然就会到来。现在让我们看看我们的计费应用需要测试什么。第 [5 章](05.html)中的 Vue 前端调用以下端点:
