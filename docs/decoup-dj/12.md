...
安装完成后，我们更新`requirements/base.txt`以包含新的依赖项。接下来，我们在`decoupled_dj/settings/base.py`中启用 Strawberry，如清单 [12-1](#PC3) 所示。
...
`typing`是主要的 Python 模块，从中我们可以仔细阅读最常见的类型声明。接下来是`strawberry`本身。我们还需要`decimal`模块和精华`datetime`。接下来，我们准备创建我们的第一个类型。清单 [12-4](#PC6) 显示了 Strawberry 中的三种 GraphQL 类型。
