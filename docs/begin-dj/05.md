...
在现实世界中启动 Django 应用首先要做的事情之一是将变量`DEBUG`改为`False`。我在前面的章节中已经简单地提到了Django在从`DEBUG=False`切换到`DEBUG=True`时的行为变化。所有这些与`DEBUG`变量相关的行为变化都是为了增强项目的安全性。表 [5-1](#Tab1) 说明了使用`DEBUG=False`和`DEBUG=True`运行项目的区别。
