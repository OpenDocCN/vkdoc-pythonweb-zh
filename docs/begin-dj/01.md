...
有时更改 Django 开发 web 服务器的默认地址和端口很方便。这可能是因为默认端口正被另一个服务占用，或者需要将 web 服务器绑定到非本地地址，以便远程计算机上的某个人可以查看开发服务器。这很容易通过将端口或完整地址:端口字符串附加到`python manage.py runserver`命令来实现，如列表 [1-13](#Par123) 中的各种示例所示。
